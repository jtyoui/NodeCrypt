...
import {addSystemMsg, renderChatArea, updateChatInputStyle} from './chat.js';
...
export function renderRooms(activeId = 0) {
    ...
    roomsData.forEach((rd, i) => {
        const div = createElement('div', {
            class: 'room' + (i === activeId ? ' active' : ''), onclick: () => switchRoom(i)
        });
    ...
    if (msgType.startsWith('file_')) {
        ...
        if (msgType === 'file_start' || msgType === 'file_start_private') {
            ...
            if (fileId) { // Only proceed if we have a fileId
                const messageAlreadyInHistory = newRd.messages.some(m => m.msgType === historyMsgType && m.text && m.text.fileId === fileId && m.userName === realUserName);
