...
import {downloadFile, handleFileMessage, setupFileSend} from './util.file.js';
...
import {closeSettingsPanel, initSettings, notifyMessage, openSettingsPanel} from './util.settings.js';
...
import {activeRoomIndex, joinRoom, roomsData} from './room.js';

// 从 chat.js 中导入聊天功能相关的函数
// Import chat-related functions from chat.js
import {addMsg, addOtherMsg, addSystemMsg, autoGrowInput, setupImagePreview, setupInputPlaceholder} from './chat.js';
                ...
                if (rd.privateChatTargetId) {
                    ...
                    if (targetClient && targetClient.shared) {
                        const clientMessagePayload = {
                            a: 'm', t: 'image_private', d: messageContent
                        };
                        ...
                        const serverRelayPayload = {
                            a: 'c', p: encryptedClientMessage, c: rd.privateChatTargetId
                        };
        ...
        onSend: (message) => {
            ...
            if (rd && rd.chat) {
                ...
                if (rd.privateChatTargetId) {
                    ...
                    if (targetClient && targetClient.shared) {
                        const clientMessagePayload = {
                            a: 'm', t: msgWithUser.type + '_private', d: msgWithUser
                        };
                        const encryptedClientMessage = rd.chat.encryptClientMessage(clientMessagePayload, targetClient.shared);
                        const serverRelayPayload = {
                            a: 'c', p: encryptedClientMessage, c: rd.privateChatTargetId
